...
export const getAllProduct = async (search, limit) => {
  ...
  if (search?.length > 0) {
    res = await axios.get(
      `${process.env.REACT_APP_API_URL_BACKEND}/product/get-all?filter=name&filter=${search}&limit=${limit}`,
    );
  } else {
    res = await axios.get(
      `${process.env.REACT_APP_API_URL_BACKEND}/product/get-all?limit=${limit}`,
    );
  }
...
export const deleteManyProduct = async (data, access_token) => {
  const res = await axiosJWT.delete(
    ...
    {
      data: data, // BẮT BUỘC phải có chữ data ở đây
      headers: {
        token: `Bearer ${access_token}`,
      },
    },
...
export const getProductOfType = async (type, page, limit) => {
  if (type) {
    const res = await axios.get(
